...
public class FileManager {
	...
	// 리턴값 : 서버에 저장된 새로운 파일명(예: 2020120809271535243254235235234.png), 강아지.png 에 대한 실제 디스크에 올라가는 파일명
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
		// 해당 파라미터에 있는 값을 주면 file 을 upload 하겠다.
		
		String newFilename = null;
		
		if(bytes == null) {	// bytes 가 null 이라면 내용물이 없다는 것이다.
			return null;
		}
		
		// 클라이언트가 업로드한 파일의 이름 (클라이언트의 파일명이 없다는 것이다.)
		...
		// 강.아.지.png ==> originalFilename.lastIndexOf(".") ==> 5 (맨 마지막에 나오는 . 의 위치값)
		String fileExt = originalFilename.substring(originalFilename.lastIndexOf(".")); 
		if(fileExt == null || "".equals(fileExt)) {	// 강아지 까지만 올라오고 .뒤에 확장자가 오지 않는 경우가 있다. 이때도 null 이다.
			return null;
		}
		
		// 위의 조건을 모두 충족한다면, 아래와 같이 서버에 저장할 새로운 파일명을 만든다.
		...
		//						 2022년04월29일12시18분15초........
		
		// 업로드할 경로가 존재하지 않는 경우 폴더를 생성한다. 자바에서는 폴더와 파일 모두 dir 를 쓴다.
		...
		String pathname = path + File.separator + newFilename;	// 실제 경로명
	...
	public String doFileUpload(InputStream is, String originalFilename, String path) throws Exception {
		...
		// 서버에 저장할 새로운 파일명을 만든다.
		newFilename = String.format("%1$tY%1$tm%1$td%1$tH%1$tM%1$tS", Calendar.getInstance());
		newFilename += System.nanoTime();	// 시분초..... 붙이기
		...
		String pathname = path + File.separator + newFilename;	// 저장될 파일명
		
		byte[] byteArr = new byte[1024];	// 읽어올 단위
		int size = 0;
		FileOutputStream fos = new FileOutputStream(pathname);	// 경로명과 넘어온 파일명에 빨대를 꼽느다. (fileOutputStream)
		
		while((size = is.read(byteArr)) != -1) {		// 파일사이즈가 -1 일때까지 읽어온다.
			fos.write(byteArr, 0, size);	// 0부터 읽어온 크기(size)만큼 써준다.
		}
		fos.flush();
	...
	public boolean doFileDownload(String saveFilename, String originalFilename, String path, HttpServletResponse response) {
	    ...
	    //	또는
	       	...
	       	originalFilename = new String(originalFilename.getBytes("UTF-8"),"8859_1");		// 한글 깨짐을 방지하고자 한다.
	...
	// == 파일 삭제 하기 ==
	// 경로명과 삭제할 파일명만 있으면 된다. 객체로 만든 후 존재하는지 보는 것이다. 해당객체 존재 시 삭제하도록 한다.

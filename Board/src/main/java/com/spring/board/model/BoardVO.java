...
public class BoardVO {
   ...
   // VO 에서는 String 은 모두 호환되기 때문에 일일이 int 이런식으로 쓰지 않아도 된다. 
   ...
   // DB 에서는 Number 일지라도, JAVA 에서는 String 타입으로 호환 가능.
   private String commentCount;		// 댓글수 (어차피 DB 에서 select 해서 읽어줄 뿐이다.)
   ...
   */
   private MultipartFile attach;	// 오라클에 있는 컬럼과는 관계 없음!! (아래의 3개가 컬럼과 관계있다.)
	/* form 태그에서 type="file" 인 파일을 받아서 저장되는 필드이다. 
	   ...
	   /Board/src/main/webapp/WEB-INF/views/tiles1/board/add.jsp 파일에서 input type="file" 인 name 의 이름(attach)과 
	      동일해야만 파일첨부가 가능해진다.!!!!
	 */
   private String fileName;    // WAS(톰캣)에 저장될 파일명(2022042911121535243254235235234.png) 
   private String orgFilename; // 진짜 파일명(강아지.png)  // 사용자가 파일을 업로드 하거나 파일을 다운로드 할때 사용되어지는 파일명  (유저가 올린 파일명)

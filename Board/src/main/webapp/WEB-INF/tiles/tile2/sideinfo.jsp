...
<script type="text/javascript">
   ...
   $(document).ready(function() {
      ...
      // 문서 로딩시 startshowWeather() 를 갖고와라.
   ...
   // view-source:http://localhost:9090/board/opendata/weatherXML.action 검색해서 보면서 한다. // 
   function showWeather() {

      $.ajax({
         url:"<%= request.getContextPath()%>/opendata/weatherXML.action",
         type:"GET",
         dataType:"XML",		<%-- 원래는 JSON 을 사용했지만, 공공api 에서 XML 을 사용하고 있기 때문에 XML 을 사용한다. --%>
         ...
         // 지역개수 : 96
            ...
            // console.log( $(local).text() + " stn_id:" + $(local).attr("stn_id") + " icon:" + $(local).attr("icon") + " desc:" + $(local).attr("desc") + " ta:" + $(local).attr("ta") );
         	// 서울 stn_id:108 icon:03 desc:구름많음 ta:25.2
            // 인천 stn_id:112 icon:02 desc:구름조금 ta:21.8
               
               var icon = $(local).attr("icon");
         	// icon 이 존재하지 않을 때, 없음.png 로 한다.
               if(icon == "") {
                  icon = "없음";
               }

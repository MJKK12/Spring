...
<%-- <% 는 JAVA 이므로 헷갈리지 않도록 한다.!! --%>
<%
   String ctxPath = request.getContextPath();

   // === #193. (웹채팅관련3) === 
   // === 서버 IP 주소 알아오기(사용중인 IP주소가 유동IP 이라면 IP주소를 알아와야 한다.) ===
   // 내가 쓰고 있는 IP 를 알아와야 한다.
   InetAddress inet = InetAddress.getLocalHost(); 
   String serverIP = inet.getHostAddress();
   
 // System.out.println("serverIP : " + serverIP);
 // serverIP : 192.168.0.4 (홈페이지 접속 시 header.jsp 는 읽으므로 IP 출력됨, 현재 내가쓰고 있는 IP)
...
// serverName : http://192.168.0.4:9090
%>
   <!-- 상단 네비게이션 시작 -->
    ...
    <%-- </c:if>  --%>
 		...
 		<!-- ==== 인터셉터 알아보기 ====  -->
          <li class="nav-item dropdown">
              ...
              <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                 <a class="dropdown-item" href="<%=ctxPath%>/anyone/anyone_a.action">누구나접근_A</a><!-- 누구나 보기 때문에 login 과 관계 없다. -->
